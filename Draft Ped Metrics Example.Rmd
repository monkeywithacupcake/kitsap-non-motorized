...
comb_stand <- comb %>% 
  ...
  mutate(setting = ifelse(RUCODE == 1, "Rural", 
         ...
         ped_type = sub("\\:.*", "", TYPE),
         #ped_w = as.numeric(sub(".*?([0-9]+).*", "\\1", TYPE)),
         nn = paste(sub("paved ","",tolower(ped_type)), tolower(SIDE_OF_RO),sep ="_")) %>%
  #select(RD_LOG_ID = RD_LOG_ID_R, SEGMENT_ID = SEGMENT_ID_R, FULL_NAME = FULL_NAME_R,
  #       SEGMENT_LENGTH = LENGTH_R, SW_SH_LENGTH = LENGTH, SIDE_OF_RO,
  #       distance, road_class, setting, nn, ped_w) %>%
  pivot_wider(id_cols = c(-RD_LOG_ID:-TYPE,-ped_type), #c(RD_LOG_ID, SEGMENT_ID, FULL_NAME), 
              names_from = nn, 
              values_from = WIDTH,
              values_fn = mean) %>%
