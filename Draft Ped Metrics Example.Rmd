...
Some of the segments with score of 0 are because they have shoulders when they should have sidewalks. Here's a few example segments. If you are familiar with Central Kitsap, you might recognize this on Provost - where there are wide shoulders, but the road is in the urban growth area and identified as urban; therefore, design standards suggest sidewalks are appropriate. 

```{r}

roads_0 <- comb_score2 %>%
  filter(score == 0) %>%
  select(setting, road_class, FULL_NAME_R, sidewalk_right:score)
...
roads_10 <- comb_score2 %>%
  filter(score == 10) %>%
  group_by(setting, road_class, "Road Name" = FULL_NAME_R) %>%
  summarise(segments = nice_comma(n()), 
...
getWalkingPolygonInCounty <- function(locdf, idcol) {
  ...
  #fdf <- st_transform(fdf, 9003)
  #st_intersection(fdf, st_buffer(county_no_cities,0)) 
  # doesn't do the clip off of the last part
}
library(mapboxapi)
lib_30min <- mb_isochrone(libraries,
               time = 30, "walking", id_column="NAME")
```

Kitsap County has `r nrow(libraries)` Public libraries.
...
kitsap +
  #geom_sf(data = libraries, aes(color = NAME)) +
  geom_sf(data = lib2, aes(fill = id), 
          lwd = 0, alpha = 0.3, show.legend = FALSE) 
  geom_sf(data = lib_circles, aes(fill = NAME), 
          lwd = 0, alpha = 0.3, show.legend = FALSE) +
  geom_sf(data = st_buffer(libraries, 5280), aes(fill = NAME), 
          lwd = 0, alpha = 0.3, show.legend = FALSE) +
  theme_void() +
...
In addition, many roads have shoulders when their standards suggest they should have sidewalks. Is the presence of a shoulder, though not adequate, really the same as **nothing?**
