...
[comment]: # creates a section for a smaller geo passed with details
[comment]: # loops over a number i referencing the geo in breakdowns tibble
[comment]: # called from Draft-Ped-Metrics-Example.Rmd
...
intro_text <- paste0("Kitsap County has ",
                    ifelse(!has_points, # poly only 
                           ...
                           paste0(nrow(b_pointdf), 
                                  ...
                                  "; however, only ",
                                  nrow(b_polydf),
                                  " are outside of city limits. Here they are shown with a 30 minute walking distance")
                           ),".")

title_text <- paste("Kitsap County",b_name)
subtitle_text <- ifelse(b_name %ni% c("Commissioner Districts","LAMIRDs"),
                        "only showing those in the unincorporated county","")
...
```{r bdloop-{{i}}, include=FALSE, echo=FALSE, cache = TRUE}
                                     
expanded_child <- lapply(
    b_items,
    function(x) {
      knitr::knit_expand(file = "child_single_geo_in_region.Rmd",
                                current_geo = x )
    })
