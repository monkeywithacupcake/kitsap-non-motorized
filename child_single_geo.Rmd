...
```{r} 
library(broom)
this_geo <- st_as_sf(loop_df) %>% filter(tomatch == current_geo) #"{{current_geo}}") 
...
Within this area, there are `r this_centerline_miles` centerline road miles; `r this_sw_sh_miles` miles of recorded sidewalk or shoulder (either side of road); and `r this_comb_miles` miles where we could match the recorded sidewalk or shoulder to a road segment (either side of road)^[In theory, the miles of recorded sidewalk or shoulder and matched miles could be double the centerline miles if every mile had a sidewalk or shoulder.]
...
df <- this_comb %>% st_set_geometry(., NULL) %>% 
  ...
  summarize(miles = sum(distance)/5280,
            .groups = "drop")
knitr::kable(df)
```
